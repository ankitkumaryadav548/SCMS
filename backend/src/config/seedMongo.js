...
async function seedMongo() {
  try {
    ...
    // 2. Seed Traffic Sensors if empty
    const sensorCount = await TrafficSensor.countDocuments();
    if (sensorCount === 0) {
      console.log('🌱 Seeding Traffic Sensors...');
    const sensorIds = [
    ...
    // 3. Seed Traffic Logs if empty
    const trafficLogCount = await TrafficLog.countDocuments();
    if (trafficLogCount === 0) {
      const trafficLogs = [
        { density: 80, avg_speed: 20.00, logged_at: new Date(Date.now() - 4 * 3600000) },
        { density: 85, avg_speed: 18.50, logged_at: new Date(Date.now() - 3 * 3600000) },
        { density: 70, avg_speed: 25.00, logged_at: new Date(Date.now() - 2 * 3600000) },
        { density: 75, avg_speed: 22.50, logged_at: new Date(Date.now() - 1 * 3600000) },
        { density: 35, avg_speed: 62.00, logged_at: new Date(Date.now() - 3 * 3600000) },
        { density: 45, avg_speed: 55.00, logged_at: new Date(Date.now() - 2 * 3600000) },
        { density: 40, avg_speed: 58.20, logged_at: new Date(Date.now() - 1 * 3600000) }
      ];
      await TrafficLog.insertMany(trafficLogs);
      console.log('✅ Traffic logs seeded.');
    }

    // 4. Seed Emergency Incidents if empty
    const incidentCount = await EmergencyIncident.countDocuments();
    if (incidentCount === 0) {
    ...
    const nodeLogCount = await NodeLog.countDocuments();
    if (nodeLogCount === 0) {
      const nodeLogs = [
        {
          module: 'Emergency',
